...
from modulos.artefatos import display_path, json_output_path, matrix_candidates
...
def calcular_cm_corrigido(frame: np.ndarray, threshold_percent: float = 0.5):
    """Detector simples mantido somente para ensaios legados do autotune."""
    frame_gray = frame.mean(axis=2) if frame.ndim == 3 else frame
    max_val = float(frame_gray.max())
    if max_val < 100:
        return None
    weights = frame_gray.astype(np.float32, copy=True)
    weights[weights < (max_val * threshold_percent)] = 0.0
