...
from modulos.visao.detector_ilhas import (
    backend_name,
    capture_frame,
    centro_massa,
    centro_massa_em_roi,
...
from modulos.controle.alvo_alinhamento import roi_incluindo_alvo, salvar_alvo
...
FOCO_DIR = Path(
    os.environ.get(
        ...
        os.environ.get(
            ...
            ROOT_DIR / "resultados" / "legado" / "calibracao_por_pontos",
        ),
    )
)

CAMERA_GAIN = (
    float(os.environ.get("QKD_IDS_ANALOG_GAIN", ids_config.ANALOG_GAIN))
    if backend_name() == "ids"
    else ASI_GAIN
)
EXPOSURE_SECONDS = (
    float(os.environ.get("QKD_IDS_EXPOSURE_US", ids_config.EXPOSURE_US)) * 1e-6
    if backend_name() == "ids"
