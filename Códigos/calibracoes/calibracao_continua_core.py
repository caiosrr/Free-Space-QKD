...
from controle.alvo_alinhamento import TARGET_FILENAME, roi_incluindo_alvo, salvar_alvo
from controle.cameras.backend import backend_name, connect_camera, direct_camera, disconnect_camera, set_gain
...
from foco_multiplos import centro_massa as foco
    ...
    try:
        ...
        if not activation_ok:
            ...
            for failure in fit_validation["failures"] + local_validation.get("failures", []):
                print(f"  - {failure}")
        elif input("Validacao local aprovada. Ativar esta matriz no tracker? (s/N): ").strip().lower() in {"s", "sim"}:
